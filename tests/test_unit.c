...
static void test_add_valid() {
    ...
    fprintf(f, "UnitCo\nUnitPerson\n0900000000\nunit@test.com\n");
    fclose(f);
    (void)freopen("tests/mock_input.txt", "r", stdin);
...
static void test_search_case_insensitive(){
    ...
    fprintf(f,"searchco\n"); fclose(f);
    (void)freopen("tests/mock_input.txt","r",stdin);
    search_user(); /* ต้องเจออย่างน้อย 1 */
}
...
static void test_edit_invalid_phone_email(){
    ...
    reset_db("tests/test_unit.csv");
    strcpy(db.items[0].company,"EditCo");
    strcpy(db.items[0].contact,"P");
    strcpy(db.items[0].phone,"0901234567");
    strcpy(db.items[0].email,"p@test.com");
    strcpy(db.items[0].status,"Active");
    db.count=1; save_csv();
    ...
    fprintf(f,"EditCo\nA\nPhoneNumber\n12\ny\n"); fclose(f);
    (void)freopen("tests/mock_input.txt","r",stdin);
    edit_user(); /* ไม่ควรอัปเดต */
    assert(strcmp(db.items[0].phone,"0901234567")==0);

    f=fopen("tests/mock_input.txt","w");
    fprintf(f,"EditCo\nA\nEmail\nbadmail\ny\n"); fclose(f);
    (void)freopen("tests/mock_input.txt","r",stdin);
    edit_user(); /* ไม่ควรอัปเดต */
    assert(strcmp(db.items[0].email,"p@test.com")==0);
}
